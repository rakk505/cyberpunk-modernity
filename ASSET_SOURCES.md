...
## Cyber Armorer weapon rigs

Thirty-three firearms and two mantis blades use Bedrock rigs from the TaCZ resource pack
[Cyber Armorer](https://www.curseforge.com/minecraft/customization/tacz-cyber-armorer), imported
from `Cyber Armorer-1.1.5_for114.zip`: the eleven base frames (`overture`, `unity`, `yukimura`,
`3516`, `saratoga`, `g58_dian`, `ajax`, `copperhead`, `m2038`, `carnage`, `grad`), their
twenty-two iconic variants, and `mantis_blade` plus `mantis_blade_maxtac`.

Each weapon contributes four assets verbatim - `gun_geo/<id>.geo.json`,
`gun_anim/<id>.animation.json`, `textures/item/<id>_uv.png`, and the flat inventory icon
`textures/item/<id>.png` - plus a generated `models/item/<id>_3d.json` transcribed from the
geometry and a recoloured `tech_<id>_uv.png` atlas for the Tech variant, which reuses the base
geometry and clips. `tools/import_cyber_armorer.py` performs the import and documents how
ballistics are derived. Animation clips retain their original `cyber_armorer:` sound-effect
keyframe names; those keyframes are inert here because this mod plays its own weapon sounds.

The eleven base frames were imported from an earlier release of the pack and are deliberately left
on that geometry, so their verified first-person framing does not shift under the players who
already use them.

The project owner stated on 2026-08-05 that the pack is authorized for inclusion. As elsewhere in
this file, that records the supplied provenance; it is not an independent license determination.
